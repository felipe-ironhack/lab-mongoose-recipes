...
//    and you can't save in a variable to invoke that function after like:
//       const createOneRecipe = Recipe.create
//       createOneRecipe(newRecipe) => This will throw an error
//    so you need to create things in order and you can't create a unique recipe, delete it and recreate the same recipe
//    because the execution doesn't necessarily respect the order in the PromiseArr and every promise is fired concurrently.
//    => This one I find strange, because if you do everything 
//      without an asynchronous treatment you'll get an error trying to execute in the second time,
//      maybe the execution just don't throw an error because the db is a really small one
//      and the deleteMany() finishes before the create(), but even that was the case.... it should happen without treatment.
//      => If i try .create(newRecipe) , deleteMany(), .create(newRecipe) the Promise.all() fails...
//      My conclusion:
